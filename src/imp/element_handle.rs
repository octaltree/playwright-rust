...
use crate::protocol::generated::WritableStream;
...
impl ElementHandle {
    ...
    async fn browser_context(&self) -> ArcResult<Arc<BrowserContext>> {
        Ok(self.owner_frame().await.unwrap().unwrap()
            .upgrade().unwrap()
            .page().unwrap().upgrade().unwrap()
            .browser_context()
            .upgrade().unwrap())
    }
    ...
    pub(crate) async fn set_input_file_paths(&self, args: SetInputFilePathsArgs) -> ArcResult<()> {
        ...
        if browser_context.browser().unwrap().upgrade().unwrap().is_remote() {
        ...
        let _ = send_message!(self, "setInputFilePaths", args);
        Ok(())
    }
}
